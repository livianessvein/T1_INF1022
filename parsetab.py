
# parsetab.py
# This file is automatically generated. Do not edit.
# pylint: disable=W,C,R
_tabversion = '3.10'

_lr_method = 'LALR'

_lr_signature = "ALERTA AND ARROW BOOL DEF DESLIGAR DIFUNDIR DISPOSITIVOS EM EXECUTE FIMDISPOSITIVOS ID LIGAR MSG NUM OP_LOGIC PARA QUANDO SENAOprograma : sec_dev sec_cmdsec_dev : DISPOSITIVOS ':' lista_devices FIMDISPOSITIVOSlista_devices : linha_device lista_deviceslista_devices : linha_devicelinha_device : IDlinha_device : ID '[' ID ']' sec_cmd : lista_cmdlista_cmd : cmd ';' lista_cmdlista_cmd : cmd ';' cmd : atribuicao\n| obsv_acao\n| acaoatribuicao : DEF ID '=' valorobsv_acao : QUANDO observacao ':' acaoobsv_acao : QUANDO observacao ':' acao SENAO acaoobservacao : ID OP_LOGIC valorobservacao : ID OP_LOGIC valor AND observacaovalor : NUMvalor : BOOLvalor : IDacao : EXECUTE LIGAR EM IDacao : EXECUTE DESLIGAR EM IDacao : ALERTA PARA ID ':' MSGacao : ALERTA PARA ID ':' MSG ',' IDacao : DIFUNDIR ':' MSG ARROW '[' lista_ids ']' acao : DIFUNDIR ':' MSG ID ARROW '[' lista_ids ']' lista_ids : IDlista_ids : ID ',' lista_ids"
    
_lr_action_items = {'DISPOSITIVOS':([0,],[3,]),'$end':([1,4,5,16,27,],[0,-1,-7,-9,-8,]),'DEF':([2,16,35,],[10,10,-2,]),'QUANDO':([2,16,35,],[11,11,-2,]),'EXECUTE':([2,16,29,35,50,],[12,12,12,-2,12,]),'ALERTA':([2,16,29,35,50,],[13,13,13,-2,13,]),'DIFUNDIR':([2,16,29,35,50,],[14,14,14,-2,14,]),':':([3,14,18,33,38,40,41,43,57,],[15,23,29,46,-20,-18,-19,-16,-17,]),';':([6,7,8,9,38,39,40,41,42,44,45,52,56,62,63,67,],[16,-10,-11,-12,-20,-13,-18,-19,-14,-21,-22,-23,-15,-24,-25,-26,]),'ID':([10,11,15,22,25,26,28,30,31,32,34,37,51,53,55,58,61,64,],[17,19,26,33,26,-5,38,38,44,45,48,49,19,60,-6,62,60,60,]),'LIGAR':([12,],[20,]),'DESLIGAR':([12,],[21,]),'PARA':([13,],[22,]),'=':([17,],[28,]),'OP_LOGIC':([19,],[30,]),'EM':([20,21,],[31,32,]),'MSG':([23,46,],[34,52,]),'FIMDISPOSITIVOS':([24,25,26,36,55,],[35,-4,-5,-3,-6,]),'[':([26,47,54,],[37,53,61,]),'NUM':([28,30,],[40,40,]),'BOOL':([28,30,],[41,41,]),'ARROW':([34,48,],[47,54,]),'AND':([38,40,41,43,],[-20,-18,-19,51,]),'SENAO':([42,44,45,52,62,63,67,],[50,-21,-22,-23,-24,-25,-26,]),']':([49,59,60,65,66,],[55,63,-27,67,-28,]),',':([52,60,],[58,64,]),}

_lr_action = {}
for _k, _v in _lr_action_items.items():
   for _x,_y in zip(_v[0],_v[1]):
      if not _x in _lr_action:  _lr_action[_x] = {}
      _lr_action[_x][_k] = _y
del _lr_action_items

_lr_goto_items = {'programa':([0,],[1,]),'sec_dev':([0,],[2,]),'sec_cmd':([2,],[4,]),'lista_cmd':([2,16,],[5,27,]),'cmd':([2,16,],[6,6,]),'atribuicao':([2,16,],[7,7,]),'obsv_acao':([2,16,],[8,8,]),'acao':([2,16,29,50,],[9,9,42,56,]),'observacao':([11,51,],[18,57,]),'lista_devices':([15,25,],[24,36,]),'linha_device':([15,25,],[25,25,]),'valor':([28,30,],[39,43,]),'lista_ids':([53,61,64,],[59,65,66,]),}

_lr_goto = {}
for _k, _v in _lr_goto_items.items():
   for _x, _y in zip(_v[0], _v[1]):
       if not _x in _lr_goto: _lr_goto[_x] = {}
       _lr_goto[_x][_k] = _y
del _lr_goto_items
_lr_productions = [
  ("S' -> programa","S'",1,None,None,None),
  ('programa -> sec_dev sec_cmd','programa',2,'p_programa','compiler.py',115),
  ('sec_dev -> DISPOSITIVOS : lista_devices FIMDISPOSITIVOS','sec_dev',4,'p_sec_dev','compiler.py',145),
  ('lista_devices -> linha_device lista_devices','lista_devices',2,'p_lista_devices_rec','compiler.py',149),
  ('lista_devices -> linha_device','lista_devices',1,'p_lista_devices_base','compiler.py',153),
  ('linha_device -> ID','linha_device',1,'p_linha_device1','compiler.py',157),
  ('linha_device -> ID [ ID ]','linha_device',4,'p_linha_device2','compiler.py',161),
  ('sec_cmd -> lista_cmd','sec_cmd',1,'p_sec_cmd','compiler.py',169),
  ('lista_cmd -> cmd ; lista_cmd','lista_cmd',3,'p_lista_cmd_rec','compiler.py',173),
  ('lista_cmd -> cmd ;','lista_cmd',2,'p_lista_cmd_base','compiler.py',177),
  ('cmd -> atribuicao','cmd',1,'p_cmd','compiler.py',181),
  ('cmd -> obsv_acao','cmd',1,'p_cmd','compiler.py',182),
  ('cmd -> acao','cmd',1,'p_cmd','compiler.py',183),
  ('atribuicao -> DEF ID = valor','atribuicao',4,'p_atribuicao','compiler.py',190),
  ('obsv_acao -> QUANDO observacao : acao','obsv_acao',4,'p_obsv_acao_simples','compiler.py',201),
  ('obsv_acao -> QUANDO observacao : acao SENAO acao','obsv_acao',6,'p_obsv_acao_senao','compiler.py',205),
  ('observacao -> ID OP_LOGIC valor','observacao',3,'p_observacao_simples','compiler.py',212),
  ('observacao -> ID OP_LOGIC valor AND observacao','observacao',5,'p_observacao_and','compiler.py',217),
  ('valor -> NUM','valor',1,'p_valor_num','compiler.py',225),
  ('valor -> BOOL','valor',1,'p_valor_bool','compiler.py',229),
  ('valor -> ID','valor',1,'p_valor_id','compiler.py',233),
  ('acao -> EXECUTE LIGAR EM ID','acao',4,'p_acao_ligar','compiler.py',241),
  ('acao -> EXECUTE DESLIGAR EM ID','acao',4,'p_acao_desligar','compiler.py',245),
  ('acao -> ALERTA PARA ID : MSG','acao',5,'p_acao_alerta1','compiler.py',249),
  ('acao -> ALERTA PARA ID : MSG , ID','acao',7,'p_acao_alerta2','compiler.py',253),
  ('acao -> DIFUNDIR : MSG ARROW [ lista_ids ]','acao',7,'p_acao_difundir1','compiler.py',258),
  ('acao -> DIFUNDIR : MSG ID ARROW [ lista_ids ]','acao',8,'p_acao_difundir2','compiler.py',263),
  ('lista_ids -> ID','lista_ids',1,'p_lista_ids_unico','compiler.py',270),
  ('lista_ids -> ID , lista_ids','lista_ids',3,'p_lista_ids_multi','compiler.py',274),
]
